...
export class ScheduleServices {
  async getMovieSchedulesById(id: string): Promise<any> {
    try {
      const [movieSchedule] = await db
        .select()
        .from(movieSchedules)
        .leftJoin(movies, eq(movieSchedules.movieId, movies.id))
  ...
  async createMovieSchedule(
    movieScheduleData: MovieSchedules,
  ): Promise<MovieSchedules> {
    try {
      const [movie] = await db
        .select()
        .from(movies)
        .where(eq(movies.id, movieScheduleData.movieId));
      if (!movie) {
        throw new Error("Movie not found");
      }

      const theaterResponse = await fetch(
        `${process.env.THEATER_SERVICE_URL}/${movieScheduleData.theaterId}`,
      );
      if (!theaterResponse.ok) {
        return Promise.reject(theaterResponse);
      }

      // 👈 Cast the JSON parsed output to 'any' or an explicit object structure
      const theaterData = (await theaterResponse
        .json()
        .catch(() => null)) as any;

      if (
        !theaterData ||
        theaterData.totalScreens < movieScheduleData.screenNumber
      ) {
        throw new Error("Theres no screen at that number in this theater");
      }

      const [movieSchedule] = await db
        .insert(movieSchedules)
        .values(movieScheduleData)
        .returning();
  ...
  async deleteMovieSchedule(id: string) {
    try {
      const [deletedMovieSchedule] = await db
        .delete(movieSchedules)
        .where(eq(movieSchedules.id, id))
        .returning();
  ...
  async updateMovieSchedule(id: string, movieScheduleData: Partial<any>) {
    // Gunakan Partial untuk mendukung pembaruan sebagian
    try {
      const [updatedMovieSchedule] = await db
  ...
  async getMovieSchedulesByTheaterId(
    theaterId: string,
  ): Promise<MovieSchedules[]> {
    try {
      const movieSchedule = await db
        .select()
        .from(movieSchedules)
        .where(eq(movieSchedules.theaterId, theaterId));
      return movieSchedule;
    } catch (error) {
