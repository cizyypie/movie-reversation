...
export const scheduleRoutes = new Elysia({ prefix: "/api/schedules" })
  ...
  .get(
    ...
    async ({ params, set }) => {
      ...
      if (!schedule) {
        set.status = 404; 
        return { message: "Schedule movie not found" };
      }
      return schedule;
    },
    {
  ...
  .get(
    ...
    async ({ params }: any) =>
      scheduleService.getMovieSchedulesByMovieId(params.movieId),
    {
  ...
  .get(
    "/date/:date",
    async ({ params }: any) =>
      scheduleService.getMovieScheduleByDate(new Date(params.date)),
    {
      beforeHandle: [authMiddleware],
      params: t.Object({
  ...
  .get(
    "/theater/:theaterId",
    async ({ params }: any) =>
      scheduleService.getMovieSchedulesByTheaterId(params.theaterId),
    {
      beforeHandle: [authMiddleware],
      params: t.Object({
  ...
  .post(
    ...
    async ({ body }: any) => {
      return {
        ...
        schedule: await scheduleService.createMovieSchedule(body),
      };
    },
    {
      beforeHandle: [authMiddleware],
  ...
  .put(
    ...
    async ({ params, body }: any) =>
      scheduleService.updateMovieSchedule(params.id, body),
    {
      beforeHandle: [authMiddleware],
      params: t.Object({
  ...
  .delete(
    "/:id",
    async ({ params }: any) => scheduleService.deleteMovieSchedule(params.id),
    {
      beforeHandle: [authMiddleware],
      params: t.Object({
        id: t.String(), // t.Number() lebih baik jika id harus angka
